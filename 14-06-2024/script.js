...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Controlla lo stato di accesso ogni volta che viene eseguita un'azione
    const checkLoggedInStatus = () => {
        if (!isLoggedIn()) {
            // Utente non autenticato, mostra un messaggio o reindirizza alla pagina di login
            alert('Devi effettuare il login per utilizzare l\'applicazione.');
            return false;
        }
        return true;
    };

    // Verifica se l'utente è loggato utilizzando il localStorage
    const isLoggedIn = () => {
        return localStorage.getItem('loggedIn') === 'true';
    };

    // Event listener per il login
    buttonLoginEl.addEventListener('click', async (e) => {
        e.preventDefault();

        const username = loginFormEl.querySelector('.username').value;

        // Simuliamo un login semplice
        if (username === 'admin') {
            localStorage.setItem('loggedIn', true);
            loginFormEl.style.display = 'none'; // Nasconde il form di login
            mainContainerEl.classList.remove('hidden'); // Mostra il contenuto principale 
        } else {
            handleFeedback(new Error("Credenziali non valide"), "Credenziali non valide");
        }
    });
